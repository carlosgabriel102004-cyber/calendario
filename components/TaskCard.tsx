...
const TaskCard: React.FC<TaskCardProps> = ({ task, onToggle, onClick, availableTags }) => {
  ...
  const priorityStyles = {
    high: { border: '#ef4444', text: 'text-rose-700' }, // Vermelho
    medium: { border: '#f59e0b', text: 'text-amber-700' }, // Amarelo
    low: { border: '#10b981', text: 'text-emerald-700' }, // Verde
  };
  const pStyle = priorityStyles[task.priority] || priorityStyles.medium;

  // Cor da etiqueta (usada para o fundo claro e bordas secundárias)
    ...
    >
      <div className="flex items-start gap-2 h-full">
        ...
        <div className="flex flex-col flex-grow min-w-0">
          <span className={`font-bold line-clamp-2 leading-tight break-words ${pStyle.text} ${task.completed ? 'line-through opacity-70' : ''}`}>
            {task.title}
          </span>
          
          <div className="flex flex-wrap items-center gap-1 mt-0.5">
            {height >= 40 && <span className="text-[10px] font-semibold opacity-80" style={{ color: pStyle.text }}>{task.startTime}</span>}
            
            {/* Indicador de que a nota tem descrição */}
            {task.description && height >= 40 && (
              <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 opacity-70" style={{ color: pStyle.text }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
            )}
          </div>
        </div>
      </div>
